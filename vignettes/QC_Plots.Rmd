...
all_female.query <- hcabm40k.SeuratData::hcabm40k
load("/Users/osman/Desktop/All_female_samples/all_female_cortex_labeled.RData")
# Add pseudo timepoint variable just for this vignette
all_female.query@meta.data$timepoint[all_female.query@meta.data$orig.ident == "MUT_F_E18_WB1" | all_female.query@meta.data$orig.ident == "MUT_F_E18_WB2" | all_female.query@meta.data$orig.ident == "WT_F_E18_WB1" | all_female.query@meta.data$orig.ident == "WT_F_E18_WB2"] <- "Emb"

all_female.query@meta.data$timepoint[all_female.query@meta.data$orig.ident == "WT_F_P30_CORT1" | all_female.query@meta.data$orig.ident == "WT_F_P30_CORT2" | all_female.query@meta.data$orig.ident == "MUT_F_P30_CORT1" | all_female.query@meta.data$orig.ident == "MUT_F_P30_CORT2"] <- "P30"
...
all_female.query <- Add_Mito_Ribo_Seurat(seurat_object = all_female.query, species = "Mouse")
```
...
accepted_names %>%
  kableExtra::kbl() %>%
  kableExtra::kable_styling(bootstrap_options = c("Mouse", "mouse", "Ms", "ms")) 
```
...
all_female.query <- Add_Mito_Ribo_Seurat(seurat_object = all_female.query, species = "other", mito_pattern = "regexp_pattern", ribo_pattern = "regexp_pattern")

# Using feature name lists
mito_gene_list <- c("gene1", "gene2", "etc")
ribo_gene_list <- c("gene1", "gene2", "etc")
all_female.query <- Add_Mito_Ribo_Seurat(seurat_object = all_female.query, species = "other", mito_features = mito_gene_list, ribo_features = ribo_gene_list)

# Using combination of gene lists and gene name patterns
all_female.query <- Add_Mito_Ribo_Seurat(seurat_object = all_female.query, species = "Mouse", mito_features = mito_gene_list, ribo_pattern = "regexp_pattern")
```
...
p1 <- QC_Plots_Genes(seurat_object = all_female.query, low_cutoff = 800, high_cutoff = 5500)
p2 <- QC_Plots_UMIs(seurat_object = all_female.query, low_cutoff = 1200, high_cutoff = 45000)
p3 <- QC_Plots_Mito(seurat_object = all_female.query, high_cutoff = 20)
```

```{r echo=FALSE}
# All functions contain 
p1 <- QC_Plots_Genes(seurat_object = all_female.query, low_cutoff = 800, high_cutoff = 5500, pt.size = 0.1)
p2 <- QC_Plots_UMIs(seurat_object = all_female.query, low_cutoff = 1200, high_cutoff = 45000, pt.size = 0.1)
p3 <- QC_Plots_Mito(seurat_object = all_female.query, high_cutoff = 20, pt.size = 0.1)
```
...
p1 <- QC_Plots_UMIs(seurat_object = all_female.query, low_cutoff = 1200, high_cutoff = 45000, pt.size = 0.1)
p2 <- QC_Plots_UMIs(seurat_object = all_female.query, low_cutoff = 1200, high_cutoff = 45000, pt.size = 0.1, y_axis_log = TRUE)
...
```{r eval=FALSE}
# All functions contain 
QC_Plot_UMIvsGene(seurat_object = all_female.query, low_cutoff_gene = 800, high_cutoff_gene = 5500, low_cutoff_UMI = 500, high_cutoff_UMI = 50000)
QC_Plot_GenevsFeature(seurat_object = all_female.query, feature1 = "percent_mito", low_cutoff_gene = 800, high_cutoff_gene = 5500, high_cutoff_feature = 20)
```

```{r echo=FALSE, fig.height=7, fig.width=13, fig.align='center'}
# All functions contain 
p1 <- QC_Plot_UMIvsGene(seurat_object = all_female.query, low_cutoff_gene = 400, high_cutoff_gene = 5500, low_cutoff_UMI = 1200, high_cutoff_UMI = 45000, x_axis_label = "UMIs per Cell")
p2 <- QC_Plot_GenevsFeature(seurat_object = all_female.query, feature1 = "percent_mito", low_cutoff_gene = 800, high_cutoff_gene = 5500, high_cutoff_feature = 20)
...
```{r eval=FALSE}
QC_Plot_UMIvsGene(seurat_object = all_female.query, meta_gradient_name = "percent_mito", low_cutoff_gene = 800, high_cutoff_gene = 5500, high_cutoff_UMI = 45000)
QC_Plot_UMIvsGene(seurat_object = all_female.query, meta_gradient_name = "percent_mito", low_cutoff_gene = 800, high_cutoff_gene = 5500, high_cutoff_UMI = 45000, meta_gradient_low_cutoff = 20)
```
...
p1 <- QC_Plot_UMIvsGene(seurat_object = all_female.query, meta_gradient_name = "percent_mito", low_cutoff_gene = 800, high_cutoff_gene = 5500, high_cutoff_UMI = 45000)
p2 <- QC_Plot_UMIvsGene(seurat_object = all_female.query, meta_gradient_name = "percent_mito", low_cutoff_gene = 800, high_cutoff_gene = 5500, high_cutoff_UMI = 45000, meta_gradient_low_cutoff = 20)

wrap_plots(p1, p2, ncol = 2)
```

### Combination Plots
If you are interested in viewing `QC_Plot_UMIvsGene` both by discrete timepointing variable and by continuous variable without writing function twice you can use `combination = TRUE` and plot output will contain both plots.
...
QC_Plot_UMIvsGene(seurat_object = all_female.query, meta_gradient_name = "percent_mito", low_cutoff_gene = 800, high_cutoff_gene = 5500, high_cutoff_UMI = 45000, meta_gradient_low_cutoff = 20, combination = TRUE)
```
...
```{r}
median_stats <- Median_Stats(seurat_object = all_female.query, group_by_var = "orig.ident")
```
...
```{r eval=FALSE}
median_stats <- Median_Stats(seurat_object = all_female.query, group_by_var = "orig.ident", median_var = "meta_data_column_name")
```
...
```{r eval=FALSE}
Plot_Median_Genes(seurat_object = all_female.query, group_by = "timepoint")
Plot_Median_UMIs(seurat_object = all_female.query, group_by = "timepoint")
Plot_Median_Mito(seurat_object = all_female.query, group_by = "timepoint")
Plot_Median_Other(seurat_object = all_female.query, median_var = "percent_ribo", group_by = "timepoint")
```


```{r echo=FALSE, fig.align='center', fig.height=10, fig.width=13}
p1 <- Plot_Median_Genes(seurat_object = all_female.query, group_by = "timepoint")
p2 <- Plot_Median_UMIs(seurat_object = all_female.query, group_by = "timepoint")
p3 <- Plot_Median_Mito(seurat_object = all_female.query, group_by = "timepoint")
p4 <- Plot_Median_Other(seurat_object = all_female.query, median_var = "percent_ribo", group_by = "timepoint")
...
Plot_Cells_per_Sample(seurat_object = all_female.query, group_by = "predicted.id")
```
